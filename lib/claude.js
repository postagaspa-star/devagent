...
const TOOLS = [
  ...
  {
    name: 'run_bash',
    description: 'Run a bash command in the project directory. Use for npm install, git init, mkdir, running scripts, etc.',
    input_schema: {
      type: 'object',
      properties: {
        command: {
...
export class ClaudeClient {
   ...
   * @param {Array}    toolCalls      - tool_use blocks from Claude
   * @param {string}   projectPath    - absolute project root (used when no bridge)
   * @param {Function} onProgress     - optional (stage, detail, preview) => void callback
   * @param {Function} bridgeExecute  - optional async (command, payload) => result via bridge
   */
  async executeToolCalls(toolCalls, projectPath, onProgress = null, bridgeExecute = null) {
    const results = [];
    ...
    for (const toolUse of toolCalls) {
      ...
      // Emit progress BEFORE executing so the user sees it in real-time
      if (onProgress) {
        if      (name === 'write_file')   onProgress('file_write',   input.path, input.content?.slice(0, 120));
        else if (name === 'read_file')    onProgress('file_read',    input.path);
        else if (name === 'delete_file')  onProgress('file_delete',  input.path);
        else if (name === 'list_files')   onProgress('file_read',    input.path || '.');
        else if (name === 'search_files') onProgress('file_read',    `search: ${input.pattern}`);
        else if (name === 'run_bash')     onProgress('bash',         input.command);
      }

      try {
        if (bridgeExecute) {
          // ── Route every tool call through the local bridge ──────────────
          switch (name) {
            case 'read_file':
              result = await bridgeExecute('READ_FILE', { path: input.path });
              break;
            case 'write_file':
              result = await bridgeExecute('WRITE_FILE', { path: input.path, content: input.content });
              break;
            case 'list_files':
              result = await bridgeExecute('LIST_DIR', { path: input.path || '.', recursive: input.recursive || false });
              break;
            case 'delete_file':
              result = await bridgeExecute('DELETE_FILE', { path: input.path });
              break;
            case 'search_files':
              result = await bridgeExecute('SEARCH_FILES', { pattern: input.pattern, path: input.path || '.' });
              break;
            case 'run_bash':
              result = await bridgeExecute('EXEC_COMMAND', { command: input.command });
              break;
            default:
          ...
          switch (name) {
            ...
            case 'search_files':
              ...
              break;
            case 'run_bash':
              result = await this.toolRunBash(projectPath, input.command);
              break;
            default:
              result = { error: `Unknown tool: ${name}` };
          }
  ...
  /**
   * Tool: Run a bash command inside the project directory
   */
  async toolRunBash(projectPath, command) {
    try {
   ...
   */
  async developCode(objective, projectConfig, history = [], options = {}, bridgeExecute = null) {
    const messages = [
      ...history,
      {
        role: 'user',
...
Start by listing files to understand the project structure, then proceed with implementation.`
    ...
    while (continueLoop && iterations < maxToolIterations) {
      ...
      if (toolUses.length > 0) {
        // Execute tool calls (pass onProgress for real-time UI updates)
        const be = options.bridgeExecute || bridgeExecute || null;
        const toolResults = await this.executeToolCalls(toolUses, projectConfig.path, options.onProgress || null, be);
