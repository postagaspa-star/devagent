...
export class ClaudeClient {
   ...
   * @param {Function} onProgress  - optional (stage, detail) => void callback
   */
  async executeToolCalls(toolCalls, projectPath, onProgress = null) {
    const results = [];
...
Start by listing files to understand the project structure, then proceed with implementation.`
    ...
    while (continueLoop && iterations < maxToolIterations) {
      ...
      if (toolUses.length > 0) {
        ...
        const toolResults = await this.executeToolCalls(toolUses, projectConfig.path, options.onProgress || null);
